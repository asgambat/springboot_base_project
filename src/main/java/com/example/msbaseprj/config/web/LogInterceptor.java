...
public class LogInterceptor implements HandlerInterceptor {
	...
	private void setSpanHeader(HttpServletResponse response) {
		...
		var span = getSpan();
		if (span != null) {
			logger.trace("Setting trace headers: traceId={}, spanId={}", span.context().traceId(),
					span.context().spanId());
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		...
		logger.info("Completed: " + request.getMethod() + " " + request.getRequestURI() + " [" + response.getStatus()
				+ "] in " + duration + "ms");
	}
