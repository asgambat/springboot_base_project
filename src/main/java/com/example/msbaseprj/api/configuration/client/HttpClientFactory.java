...
public class HttpClientFactory {
    ...
    public static HttpClient create(HttpClientCustomizer customizer) {
        var evictionIdleTimeSec = customizer.getEvictionIdleTimeoutSeconds(); //idle time after which connections in the pool can be evicted

        return HttpClients.custom()
                            .setDefaultRequestConfig(createRequestConfig(customizer))
                            .setConnectionManager(createConnectionManager(customizer))
                            .evictExpiredConnections()
                            .evictIdleConnections(Timeout.ofSeconds(evictionIdleTimeSec))
                            .disableAutomaticRetries() //we handle retries at the service level with Spring Retry, so we disable the default HttpClient retry mechanism to avoid unexpected interactions between the two
                            .build();
    }

    private static RequestConfig createRequestConfig(HttpClientCustomizer customizer) {
        var connRequestTimeoutMillis = customizer.getConnectionPoolMaxWaitTimeMillis(); //max time to wait for a connection from the pool
        var responseTimeoutMillis = customizer.getResponseTimeoutMillis(); //response timeout aka read timeout

        return RequestConfig.custom()
                            .setConnectionRequestTimeout(Timeout.ofMilliseconds(connRequestTimeoutMillis))
    ...
    private static ConnectionConfig createConnectionConfig(HttpClientCustomizer customizer) {
        ...
        return ConnectionConfig.custom()
                            ...
                            .setTimeToLive(Timeout.ofSeconds(connTtlSec))
                            .build();  
    }
}
