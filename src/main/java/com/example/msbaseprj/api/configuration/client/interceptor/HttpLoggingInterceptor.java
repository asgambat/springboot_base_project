...
public class HttpLoggingInterceptor implements ClientHttpRequestInterceptor {
    ...
    private void logRequest(HttpRequest request, byte[] body) {
        ...
        //request.getHeaders().forEach((key, value) -> log.info("HTTP REQUEST HEADER >> {}={}", key, String.join(",", value)));

        if( body.length > 0 ) 
            log.debug("HTTP REQUEST BODY >> {}", new String(body, StandardCharsets.UTF_8));
    }
    ...
    private ClientHttpResponse logResponse(ClientHttpResponse response) throws IOException {
        ...
        if( log.isTraceEnabled() )
            response.getHeaders().forEach((key, value) -> log.trace("HTTP RESPONSE HEADER << {}={}", key, String.join(",", value)));

        if( log.isDebugEnabled() ) {
            try {
                response = new BufferingClientHttpResponseWrapper(response);
                //we read max 10KB of response body, to avoid filling logs with huge responses, but enough for debugging purposes
                ...
                else log.debug("HTTP RESPONSE BODY << {}", responseBody);
            } catch(Exception e) {
