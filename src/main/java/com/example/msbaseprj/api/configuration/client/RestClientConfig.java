...
import com.example.msbaseprj.api.exception.TooManyRequestException;
...
public class RestClientConfig {
    ...
    RestClient restClient( ) {        
         return createBaseRestClient(externalClientProperties.getApi().isEnableLoggingInterceptor())
                         ...
                         .requestFactory(createRequestFactory(externalClientProperties.getConnection()))
                         .build();
    }
    ...
    private ErrorHandler tooManyRequestsHandler() {
        return (status, response) -> {
            long retryAfterMillis = _defaultRetryDelayMillis; // default retry after 2 seconds if header is not present or cannot be parsed
            try {
            ...
            throw new TooManyRequestException("Too many requests", retryAfterMillis);
        };  
    }
